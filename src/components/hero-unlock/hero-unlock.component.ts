...
export class HeroUnlockComponent {
  ...
  // Removed unlockCost as the hero is already unlocked/paid for by the time this screen is shown.
  // The button's purpose is to navigate and celebrate, not perform the unlock action.

  viewHeroInRoster() {
    // The hero is already unlocked by the summoning process.
    // This button just navigates to the team view.
  ...
  getRarityGlowClass(rarity: Rarity): string {
    switch (rarity) {
      case 'Common': return 'animate-hero-glow-rarity-Common';
      case 'Rare': return 'animate-hero-glow-rarity-Rare';
      case 'Epic': return 'animate-hero-glow-rarity-Epic';
      case 'Legendary': return 'animate-hero-glow-rarity-Legendary';
      case 'Mythic': return 'animate-hero-glow-rarity-Mythic';
      default: return '';
    }
  }
}