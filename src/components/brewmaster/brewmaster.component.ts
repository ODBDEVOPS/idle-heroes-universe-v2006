...
@Component({
  selector: 'app-brewmaster',
  standalone: true,
  template: `
    <div class="h-full w-full relative overflow-hidden flex flex-col p-4 bg-gray-900 animate-fadeIn">
      <div class="absolute inset-0 bg-gradient-to-b from-gray-900 via-amber-900/40 to-gray-900 opacity-50"></div>
      ...
      <header class="relative z-10 text-center mb-4">
        ...
        <h1 class="text-4xl font-orbitron text-amber-300 drop-shadow-lg">Brewmaster</h1>
      </header>
       <main class="relative z-10 flex-grow grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pr-2">
        <div class="bg-black/30 p-4 rounded-xl border border-gray-700/50 flex flex-col">
          <h2 class="text-xl font-orbitron text-gray-300 text-center mb-3">Your Ingredients</h2>
          <div class="flex-grow overflow-y-auto space-y-2">
            ...
            @if(!hasMaterials()) {
              <p class="text-center text-gray-500 italic py-8">You have no brewing ingredients.</p>
            }
          </div>
        </div>
        <div class="bg-black/30 p-6 rounded-xl border border-gray-700/50 flex flex-col items-center justify-center text-center">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-24 w-24 mx-auto text-gray-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 7.5l.415-.207a.75.75 0 0 1 1.085.67V10.5m0 0h6m-6 0a.75.75 0 0 0-1.085.67l-.415.207a.75.75 0 0 0-.67 1.085l.207.415a1.5 1.5 0 0 0 2.17 0l.415-.207a.75.75 0 0 0 .67-1.085l-.207-.415a.75.75 0 0 0-1.085-.67Z" /></svg>
          <h2 class="text-2xl font-orbitron text-gray-500">Brewing Kettle</h2>
          <p class="text-gray-400 mt-2">Recipes for potent beers, wines, and spirits that grant powerful temporary buffs will be available here soon.</p>
        </div>
      </main>
    </div>
  `,
...
export class BrewmasterComponent {
  ...
  materials = computed(() => {
    ...
    return ALL_MATERIALS
      .filter(m => m.type === 'Herb' || m.type === 'Fish')
      .map(material => ({
        ...material,
